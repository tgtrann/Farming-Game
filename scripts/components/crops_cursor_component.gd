...
var corn_plant_scene = preload("res://scenes/objects/corn.tscn")
var tomato_plant_scene = preload("res://scenes/objects/tomato.tscn")
...
##the function instantiates the associated sprites 
func add_crop() -> void:
	if distance < 20.0:
		if ToolManager.selected_tool == DataTypes.Tools.PlantCorn:
			...
			#get's the parent node of the scene, finds the child node called 
			#CropFields and add the child to the CropFields node
