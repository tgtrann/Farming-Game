...
var balloon_scene = preload("res://scenes/dialogue/game_dialogue_balloon.tscn")
...
func _ready() -> void:
	...
	# food_received = signal variable
	# on_food_received = method that gets called when food_received calls emit()
...
func _unhandled_input(event: InputEvent) -> void:
	if in_range:
		if event.is_action_pressed("show_dialogue"):
			...
			balloon.start(load("res://scenes/dialogue/conversations/chest.dialogue"), dialogue_start_command)
...
func add_reward_scene() -> void:
	for scene in output_reward_scenes:
		...
		#calculate the random position for the reward
