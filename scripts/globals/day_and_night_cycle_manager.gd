...
##the function sets the initial time at game launch
func _ready() -> void:
	set_initial_time()
	
##this controls how much time passes based on game speed 
func _process(delta: float) -> void:
	time += delta * game_speed * GAME_MINUTE_DURATION
	game_time.emit(time)
	
	#invoke method for calculating time
	recalculate_time()

#this function calculates the initial time for the day	
...
##the function calculates time for the day, minute, hour and current day
